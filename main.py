...
def run_discover(subnet: str = "192.168.1.0/24"):
    ...
    if res["hosts"]:
        ...
        print(f" {'IP ADDRESS':<16} {'HOSTNAME':<22} {'OS GUESS (UNRELIABLE)':<40} {'DEVICE TYPE':<26} {'OPEN PORTS'}")
        print(f" {'-'*14:<16} {'-'*20:<22} {'-'*38:<40} {'-'*24:<26} {'-'*15}")
        for h in res["hosts"]:
            ports_str = ",".join(map(str, h["open_ports"])) if h["open_ports"] else "None"
            os_guess = h.get("os_guess", "Unknown")
            print(f" {h['ip']:<16} {Colors.BOLD}{h['hostname']:<22}{Colors.ENDC} {os_guess:<40} {h['device_type']:<26} {ports_str}")
        print()
    else:
